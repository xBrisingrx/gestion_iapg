class CertificatesController < ApplicationController
  ...
  def courses_by_type
    @courses = Course.where(course_type_id: params[:course_type_id]).includes(:room).order(:from_date)
  end

  def get_people_in_course
    @query = CoursePerson.where(course_id: params[:course_id], company_id: params[:company_id]).ransack(params[:query])
